...
print("✅ FAISS index and policy chunks loaded.")
...
# ==== SHAP-Compatible Prediction ====
def predict_proba_shap(texts):
    str_texts = [str(t) for t in texts]
    encodings = tokenizer(str_texts, return_tensors="pt", padding=True, truncation=True, max_length=512)
    with torch.no_grad():
        output = model(**encodings)
        probs = torch.nn.functional.softmax(output.logits, dim=1)
    return probs.detach().numpy()

# ==== SHAP Explanation with Context ====
...
def predict_severity(request: TextRequest):
    ...
    retrieved_chunks = retrieve_relevant_chunks(text)
